...
// 조작 도움말 졸업제(정보 다이어트 2026-07) — 완주 8회 뒤에는 좌하 키 레전드를
// 숨긴다. 조작을 익힌 뒤엔 상시 노이즈일 뿐이고, 맥락 힌트(.hint)는 남는다.
const PLAYS_KEY = 'btb:plays';
function playCount() {
  try { return parseInt(localStorage.getItem(PLAYS_KEY), 10) || 0; } catch { return 0; }
}
function bumpPlayCount() {
  try { localStorage.setItem(PLAYS_KEY, String(playCount() + 1)); } catch { /* private */ }
  applyHelpGraduation();
}
function applyHelpGraduation() {
  document.querySelector('.actionbar')?.classList.toggle('kb-graduated', playCount() >= 8);
}
applyHelpGraduation();
...
function showCareerResult({ tone, score, income, prog, oppName, mission, seasonGoals = [], cond, report, identity, training = [], rivalBonus = 0, cupNote = null }) {
  ...
  if (reportEl) {
    // 'details': 코치 한 줄은 직전 결말 카드가 이미 말했다 — 여기선 지표·분석만.
    reportEl.innerHTML = renderTacticalReport(report, 'details');
...
function updateTacticalHud(s) {
  ...
  if (situationEl) {
    ...
    if (situation) {
      ...
      // 결정 제목은 상단 오브젝티브 스트립이 이미 "{title} — 지금 대응하세요"로
      // 말한다 — 여기선 설명만(같은 제목이 40px 간격 이중 노출되던 것 정리).
      document.getElementById('situation-title').textContent = s.matchDecision ? '' : situation.title;
...
// 큐 배너 TTL(정보 다이어트) — 문구가 바뀐 시점부터 4초만 노출.
const cueTTL = { text: null, at: 0 };
const cueVisible = (cue) => (cue && performance.now() - cueTTL.at < 4000 ? cue : null);
...
function loop(ts) {
  ...
  // 큐 배너 TTL(정보 다이어트 2026-07) — 상시 조언판이 아니라 순간 피드백 토스트.
  // 킥오프의 시나리오 힌트도, 이벤트 문구도 4초 보여주고 걷는다. 상시 조언은
  // CURRENT OBJECTIVE 스트립 한 채널만.
  if ((s.cue ?? null) !== cueTTL.text) { cueTTL.text = s.cue ?? null; cueTTL.at = performance.now(); }
  ...
  if (s.status === 'over' && !engine.busy && !outcomeShown) {
    ...
    bumpPlayCount();   // 도움말 졸업 카운트 — 완주(결말 도달)만 센다
    renderLog(engine);
    if (careerActive) {
      ...
      if (careerSettled) settleCareerMatch();   // 방어적 — 정상 경로에선 도달 안 함
        ...
        // reportMode 'coach': 결말 카드는 재도전/정산 판단용 코치 한 줄만 —
        // 상세 리포트는 뒤따르는 결과 카드가 보인다(연속 중복 제거).
        const openOc = () => showOutcome(engine, retryAttempt, () => settleCareerMatch(),
          { nextLabel: t('oc.settle'), onReplay: () => startReplay(openOc), reportMode: 'coach' });
        openOc();
      }
    } else {
